...
def generate_KF_cycle(F, dt, dx, n_sampling,
                      ary2_y_o, ary_x_a_0):
    ...
    for i_step in range(1, len(ary2_y_o)):
        ...
        ary2_K\
        = np.dot(
                 np.dot(ary2_B, ary2_H.T),
                 np.linalg.inv(
                               np.dot(np.dot(ary2_H, ary2_B), ary2_H.T)
                               +ary2_R
                               ),
                 )
...
def main():
    ...
    filepath_to_output = "task5_output/365-730days_per6h_dt36m_OIFA_dx"\
                         +(
                           #"10"
                           #"05"
                           "01"
