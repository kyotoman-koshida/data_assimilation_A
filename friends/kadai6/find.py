...
import time;    bgn = time.time()
import csv
path = 'C:/code/python/data-asmp/result/'

J=40;   F=8;    h=0.05  # 予報サイクル：6hおき
dt=0.01            # 計算の単位時間
a=float(input())
L = list(range(J))

D_o = np.loadtxt(path +'obs.csv', delimiter=',', dtype='float32')       # 観測値
D_t = np.loadtxt(path +'truth.csv', delimiter=',', dtype='float32')     # 真値
B = np.matrix( a* np.loadtxt(path +'B6h.csv', delimiter=',', dtype='float32') )
N = len(D_o)

def Enorm(x, t):           # Root Mean Square Error : root(sum 1~n ai^2)/n
    x = np.reshape(np.array(x), J);     x -= t
    n = len(x);     s = 0
    while n > 0:
        n -= 1;     s += x[n]**2
...
def obs_rm(n):          # n点ランダムに選んで観測する
    O = rm.choice(J, size=n, replace=False);    O.sort()
    H = np.zeros((n, J))
    for i in range(n):
        H[i, O[i]] = 1
    return np.matrix(H)
...
for j in range(5):
    L = [f'a={a}'] + [100]*(J+1)
    for n in range(J+1):
        e = 0;  x = np.matrix(D_t[rm.randint(N)])  
        for i in range(T):
            x = model(x)
            H = obs_rm(n)
            y = H * np.matrix(D_o[i+1]).T
            K = B*H.T *np.linalg.inv( H*B*H.T + err_cov(n) )
            x += ( K*(y- H*x.T) ).T
            if i > 40*4:
                e += (Enorm(x, D_t[i+1]) - e)/ (i-40*4)
        if e < L[n+1]:
            L[n+1] = e
        if n%20 == 0:    
            cur = time.time()
